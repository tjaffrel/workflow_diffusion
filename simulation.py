...
def extract_raspa_output(raspa_output, has_N2=False):
    ...
    if has_N2:
        ...
        enthalpy_of_adsorption_CO2 = (
            float(re.findall(r"(?<=\[K\])\s*-?\d*\.\d*", CO2_enthalpy_subsection)[0])
            * 0.239
        )
        ...
        enthalpy_of_adsorption_N2 = (
            float(re.findall(r"(?<=\[K\])\s*-?\d*\.\d*", N2_enthalpy_subsection)[0])
            * 0.239
        )
